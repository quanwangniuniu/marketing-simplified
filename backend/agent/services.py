...
def _call_dify(spreadsheet_data, user_id=None, success_criteria=None):
    ...
    Args:
        success_criteria: Optional dict generated by GenerateCriteriaExecutor.
                          Passed to Dify so the LLM knows what to look for in
    ...
    inputs = {'spreadsheet_data': json_input(spreadsheet_data)}
    if success_criteria:
        inputs['success_criteria'] = json_input(success_criteria)
        ...
        for key in ('result', 'text', 'output', 'analysis'):
            ...
            if isinstance(val, str):
                ...
                try:
                    ...
                    logger.debug(f"Dify output parsed from '{key}' key")
                    return parsed
                except json.JSONDecodeError:
                    pass
            elif isinstance(val, dict):
                logger.debug(f"Dify output is dict under '{key}' key")
                return val
...
def _run_analysis(spreadsheet_data, user_id=None, success_criteria=None):
    ...
    Args:
        success_criteria: Optional dict from GenerateCriteriaExecutor. Forwarded
                          to Dify so the analysis LLM knows what criteria to apply.
    """
    # 1. Try Dify first if configured
    dify_config = _get_dify_config()
    if dify_config:
        try:
            return _call_dify(spreadsheet_data, user_id, success_criteria=success_criteria)
        except Exception as e:
            logger.error(f"Dify call failed, falling back to Claude: {e}")

    # 2. Try Claude API (criteria not forwarded — Claude path is a fallback only)
